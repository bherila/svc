...
- Keep independent GitHub Actions jobs concurrent; do not serialize lanes that can safely run in parallel. The serial PHP suite needs at least 1 GiB of memory. Before enabling ParaTest, provision additional headroom and measure peak memory with the intended worker count and per-process MariaDB schemas.
