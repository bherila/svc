...
class SyntheticImportRehearsalTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_rehearsal_applies_twice_verifies_idempotency_and_removes_artifacts(): void
    {
        ...
        $temporaryDirectoriesBefore = glob(sys_get_temp_dir().'/svc-external-import-rehearsal-*') ?: [];

        $exit = Artisan::call('svc:import:external:rehearse', ['--format' => 'json']);
        ...
        $this->assertSame($temporaryDirectoriesBefore, glob(sys_get_temp_dir().'/svc-external-import-rehearsal-*') ?: []);
    ...
    public function test_rehearsal_refuses_non_disposable_environments(): void
    {
        ...
        try {
            $this->artisan('svc:import:external:rehearse', ['--format' => 'json'])
                ->assertFailed()
    ...
    public function test_rehearsal_rejects_unknown_output_formats_without_creating_artifacts(): void
    {
        ...
        $this->artisan('svc:import:external:rehearse', ['--format' => 'xml'])
            ->assertFailed()
