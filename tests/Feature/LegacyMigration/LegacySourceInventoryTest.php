...
class LegacySourceInventoryTest extends TestCase
{
    ...
    {
        Config::set('legacy-migration.destination_connection', 'deliberately-missing');
        $destinationCounts = $this->destinationCounts();

        $exit = Artisan::call('svc:migrate:legacy:inventory', ['--format' => 'json']);
    ...
    {
        Artisan::call('svc:migrate:legacy:inventory', ['--format' => 'json']);
    ...
    {
        $this->artisan('svc:migrate:legacy:inventory', ['--source' => 'unknown', '--format' => 'json'])
            ->assertFailed()
    ...
    {
        Config::set('legacy-migration.sources', []);

        $this->artisan('svc:migrate:legacy:inventory', ['--format' => 'xml'])
            ->assertFailed()
            ->expectsOutput('Legacy source inventory could not run: invalid_format');
    }
    ...
    private function destinationCounts(): array
    {
        return [
            ...
            'runs' => DB::table('legacy_migration_runs')->count(),
            'items' => DB::table('legacy_migration_items')->count(),
        ];
    }
}
