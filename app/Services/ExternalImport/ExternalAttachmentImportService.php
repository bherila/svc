...
final class ExternalAttachmentImportService
{
    ...
    public function run(string $sourceName, string $workspaceIdentifier, string $uploaderPublicId, bool $apply = false): array
    {
        ...
        $items = ExternalImportItem::query()
    ...
    private function processItem(mixed $connection, string $sourceIdentityHash, string $root, Workspace $workspace, User $uploader, ExternalImportItem $item, bool $apply): string
    {
        ...
        $publicId = Uuid::uuid5(
            Uuid::NAMESPACE_URL,
            implode(':', ['svc', 'external-attachment', $sourceIdentityHash, $item->source_table, $item->source_key]),
        )->toString();
    ...
    private function record(Workspace $workspace, string $sourceIdentityHash, array $mapping, array $row): Model
    {
        ...
        $parentItem = ExternalImportItem::query()
    ...
    private function sourceFile(string $root, array $row, bool $materialize = false): array
    {
        ...
        try {
            ...
            if ($materialize) {
                $temporaryPath = tempnam(sys_get_temp_dir(), 'svc-external-attachment-');
    ...
    private function assertExistingCopy(Workspace $workspace, Model $record, string $recordType, array $sourceFile, ExternalImportAttachmentCopy $copy, ClientAttachment $attachment): void
    {
    ...
    /** @param array{path_hash:string,sha256:string,bytes:int} $sourceFile */
    private function completeLedger(ExternalImportItem $item, Workspace $workspace, ClientAttachment $attachment, array $sourceFile): void
    {
        DB::transaction(function () use ($item, $workspace, $attachment, $sourceFile): void {
            ExternalImportAttachmentCopy::query()->create([
    ...
    {
        $configured = Config::get('external-import.attachment_root');
    ...
    {
        $destination = Config::get('external-import.destination_connection') ?: Config::get('database.default');
    ...
    private function originalFilename(array $row): string
    {
        ...
        return in_array($filename, ['', '.', '..'], true) ? 'external-attachment' : $filename;
    }
