...
final class ExternalImportService
{
    ...
    /**
     * The start date an imported agreement must state (#147).
     *
     * `client_agreements.starts_on` is `NOT NULL` because a null had seven
     * readings and several were incompatible - the rate resolver treated one as
     * in force while the timesheet's capacity query and every date-based
     * selector treated it as excluded, so an undated agreement could price work
     * it granted no capacity for. The column now has no null to disagree about,
     * and this is the edge the source could still push one through.
     *
     * It refuses rather than substituting. Every candidate default writes a
     * different billing history: today's date makes past work unpriced, the
     * earliest invoice's date invents a term the parties never agreed, and the
     * epoch grants capacity for years that never existed. An import that stops
     * on a row and names it is recoverable; one that guesses is not, because
     * nothing downstream can tell an invented date from a real one.
     *
     * @param  array<string, mixed>  $row
     */
    public static function importedAgreementStart(array $row): string
    {
        $start = self::sourceDate($row['active_date'] ?? null);
