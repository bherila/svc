...
final class SyntheticExternalSource
{
    /**
     * @return array{external_user_id: int, source_rows: int}
     */
    public function create(string $path): array
    {
        if ($path === ':memory:') {
            throw new InvalidArgumentException('Synthetic external sources require an SQLite file path, not :memory:.');
        }

        if ($path === '' || is_link($path) || (file_exists($path) && ! is_file($path))) {
            throw new InvalidArgumentException('Synthetic external sources require an exact, regular SQLite file path.');
        }

        if (file_exists($path) && filesize($path) > 0) {
            throw new InvalidArgumentException('Synthetic external source path must not contain an existing non-empty database.');
        }
