...
final class AgentMcpServerFactory
{
    ...
    private function instructions(array $available, bool $hasWriteTools): string
    {
        ...
        if ($this->hasTools($available, ['time_entries.log', 'time_entries.approve'])) {
            $mode .= ' When the user asks to approve time they are logging, pass approve: true to time_entries.log instead of a separate approve call; it returns the rows as time_entries.list does.';
        }
