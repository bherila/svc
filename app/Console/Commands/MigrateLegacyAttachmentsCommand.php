...
class MigrateLegacyAttachmentsCommand extends Command
{
    protected $signature = 'svc:migrate:legacy:attachments
    ...
    protected $description = 'Copy planned legacy attachments into private SVC storage; dry-run by default';

    public function handle(LegacyAttachmentMigrationService $migration): int
    {
        ...
        try {
            ...
            return $this->failure($format, 'attachment_migration_unavailable');
        }
    ...
    private function failure(string $format, string $reason): int
    {
        ...
        if ($format === 'json') {
            ...
            $this->error('Attachment migration could not run: '.$reason);
        }
