...
{
    protected $signature = 'svc:import:external
        {--source=external : Explicit configured source name}
        {--workspace= : Workspace public UUID or slug}
        {--apply : Permit destination writes; without this the command is a dry run}
        {--format=text : Output text or json}';

    protected $description = 'Inventory or import a configured external source; dry-run by default';

    public function handle(ExternalImportService $import): int
    {
        ...
        try {
            $summary = $import->run((string) $this->option('source'), $workspace, (bool) $this->option('apply'));
        } catch (SourceConfigurationException $exception) {
            return $this->failure($format, $exception->reasonCode);
        } catch (Throwable) {
            return $this->failure($format, 'import_unavailable');
        }
    ...
    private function failure(string $format, string $reason): int
    {
        ...
        if ($format === 'json') {
            ...
            $this->error('Import could not run: '.$reason);
        }
