...
class VerifyExternalImportCommand extends Command
{
    protected $signature = 'svc:import:external:verify
        {--run= : Import run public UUID}
        {--workspace= : Workspace public UUID or slug}
        {--format=text : Output text or json}';

    protected $description = 'Verify an external import run using redacted ledger state';
    ...
    public function handle(ExternalImportService $import): int
    {
        ...
        try {
            $summary = $import->verify((string) ($this->option('run') ?: ''), (string) ($this->option('workspace') ?: ''));
