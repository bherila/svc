...
if (expectedResource) {
    ...
    if (challenge.status !== 401) {
        throw new Error(`Unauthenticated MCP initialize returned ${challenge.status}, not a 401 challenge.`);
    }
    ...
    if (!metadataUrl) {
        throw new Error('The 401 challenge did not advertise a resource_metadata URL.');
    }

    const metadata = await fetch(metadataUrl).then((response) => {
        if (!response.ok) {
            throw new Error(`The advertised resource metadata URL returned ${response.status}.`);
        }
    ...
    if (metadata.resource !== expectedResource) {
        throw new Error('The resource metadata identifies a different protected resource than expected.');
    }
...
try {
    ...
    if (!contextTool.outputSchema) {
        throw new Error('context.get advertises no output schema, so its structured result cannot be validated.');
    }
    ...
    if (wrongScopeToken) {
        ...
        try {
            ...
            if (!refused.isError) {
                throw new Error('A connection lacking identity:read was allowed to read context.');
            }
        ...
        if (!stolen) {
            throw new Error('The authorized connection negotiated no session id to test isolation with.');
        }

        let isolated = false;

        try {
            const hijack = await connect(wrongScopeToken, { sessionId: stolen });
        ...
        if (!isolated) {
            throw new Error('A second credential reused the first connection\'s MCP session id.');
        }
